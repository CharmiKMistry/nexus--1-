        ...
        <div className={`p-4 rounded-md border ${isDark ? "bg-[#1F1F1F] border-[#2D2D2D]" : "bg-white border-[#EDEBE9] shadow-sm"}`} id="add_event_form_container">
          ...
          <form onSubmit={handleCreateEvent} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1 md:col-span-2">
              <label className="text-[10.5px] font-bold text-slate-400 block">Supporting Document Ref (Filename/URL)</label>
              <input 
                type="text" 
                placeholder="e.g., Promotion_Board_July_Approval.pdf" 
                value={formSupportDocs}
                onChange={(e) => setFormSupportDocs(e.target.value)}
                className={`w-full text-xs p-1.5 rounded border outline-none ${isDark ? "bg-[#1F1F1F] border-[#2D2D2D] focus:border-[#0078D4]" : "bg-white border-[#EDEBE9]"}`}
              />
            </div>
            ...
            {selectedEvent ? (
              <div className={`rounded-md p-4 border space-y-4 ${isDark ? "bg-[#1F1F1F] border-[#2D2D2D]" : "bg-white border-[#EDEBE9] shadow-sm"}`} id="event_details_cockpit">
                ...
                <div className="space-y-2.5 text-xs">
                  ...
                  <div className="border-b border-[#EDEBE9]/10 pb-1.5">
                    <span className="text-slate-400 font-medium block mb-1">Supporting Document Attachment</span>
                    <div className="p-1.5 bg-slate-500/5 rounded border border-slate-700/10 flex items-center gap-1.5 text-[11px] font-semibold text-[#0078D4] cursor-pointer hover:underline">
                      <FileText size={12} />
                      <span>{selectedEvent.supportingDocs}</span>
                    </div>
                  </div>
                  <div>
